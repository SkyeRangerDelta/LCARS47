...
exports.run = (lcars, msg, cmd) => { //BEGIN EXECUTION
    ...
    function play(guild, song) {
        ...
        const playerDispatch = lcars.serverQueue.connection.playStream(ytdl(song.url, {quality: 'highestaudio'}))
        .on('end', reason => {
...
/*
                uservc.join().then(connection => {
                    const playerStream = ytdl(song.url, {quality: 'highestaudio', filter: 'audioonly'});
                    const musicStream = connection.playStream(playerStream);

                    playerStream.on('end', reason => {
                        console.log("Readstream ended, reason:\n" + reason);
                    }).on('error', reason => {
                        console.log("Readstream error, reason:\n" + reason.stack);
                    });

                    musicStream.on('start', x => {
                        console.log("Stream has started.");
                    }).on('error', reason => {
                        uservc.leave();
                        console.log("Stream errored, reason:\n" + reason.stack);
                    }).on('end', reason => {
                        uservc.leave();
